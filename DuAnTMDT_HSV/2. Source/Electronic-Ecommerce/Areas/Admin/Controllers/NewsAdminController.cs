...
using static Electronic_Ecommerce.DTOs.NewsDTO;
using Domain;
    ...
    public class NewsAdminController : BaseController
    {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    switch (sortOrder)
                    {
                        ...
                        case "comment_desc":
                            ...
                            list = from a in db.News
                                   where (a.status == "1")
                                   orderby a.NewComments.Count descending
                                   select a;
                            break;
                        case "comment_asc":
                    ...
                    switch (show)
                    {
                        //tìm kiếm tất cả
                        case "1":
                            list = (IOrderedQueryable<New>)list.Where(s =>
                                s.news_id.ToString().Contains(search) || s.news_title.Contains(search));
                            break;
                        //theo id
                        case "2":
                            list = (IOrderedQueryable<New>)list.Where(s => s.news_id.ToString().Contains(search));
                            break;
                        //theo tên thể loại
                        case "3":
                            list = (IOrderedQueryable<New>)list.Where(s => s.news_title.ToString().Contains(search));
                            break;
                        //theo tên danh mục
                        case "4":
                            list = (IOrderedQueryable<New>)list.Where(s => s.ChildCategory.name.ToString().Contains(search));
                            break;
                    }
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    if (!string.IsNullOrEmpty(search))
                    {
                        ...
                        return View("NewsIndex", list.ToPagedList(pageNumber, 50) as IPagedList<New>);
                    }
        ...
        public JsonResult DeleteStickyPost(int id)
        {
            ...
            if (User.Identity.Permiss_Delete() == true)
            {
                ...
                string tittle = hotPost.New.news_title;
        ...
        public ActionResult NewsCreate()
        {
            if (User.Identity.IsAuthenticated)
            {
                if (User.Identity.Permiss_Create() == true)
                {
                    ViewBag.ListNewsCategory = new SelectList(db.ChildCategories.Where(m => (m.status == "1")).OrderBy(m => m.name),
                    ...
                    var newstagcheck = from t in db.Tags
                                       ...
                                       select new
                                       {
                                           t.tag_id,
                                           t.tag_name,
                                           Checked = ((from nt in db.NewTags where (nt.tag_id == t.tag_id) select nt).Count() > 0)
                                       };
                    var newsproductcheck = from p in db.Products
                                           ...
                                           {
                                               p.product_id,
                                               p.GenreDetail.Genre.genre_id,
                                               p.product_name,
                                               p.image,
                                               Checked = ((from np in db.NewProducts
                                                           where (np.product_id == p.product_id) && (np.genre_id == p.GenreDetail.Genre.genre_id)
                                                           select np).Count() > 0)
                                           };
        ...
        public ActionResult NewsCreate(NewsDTO newsdto, New news)
        {
            ViewBag.ListNewsCategory = new SelectList(db.ChildCategories.Where(m => (m.status == "1")).OrderBy(m => m.name), "childcategory_id", "name", 0);
            ...
            try
            {
                ...
                foreach (var item in newsdto.Tags)
                {
                    if (item.Checked)
                    {
                        db.NewTags.Add(new NewTag() { news_id = newsdto.news_id, tag_id = item.id });
                    }
                }
                foreach (var item in newsdto.Products)
                {
                    if (item.Checked)
                    {
                        db.NewProducts.Add(new NewProduct()
                        { 
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult CreateTags(Tag tags, NewsDTO newsdto)
        {
        ...
        public ActionResult NewsEdit(int? id, string returnUrl)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                New news = db.News.Where(m => m.news_id == id).FirstOrDefault();
                if ((User.Identity.Permiss_Modify() == true) && (news.status == "1" || news.status == "0"))
                {
                    ...
                    ViewBag.ListNewsCategory =
                        new SelectList(db.ChildCategories.Where(m => (m.status == "1")).OrderBy(m => m.name),
                            "childcategory_id", "name", 0);
                    var newstagcheck = from t in db.Tags
                                       orderby t.tag_name ascending
                                       select new
                                       {
                                           t.tag_id,
                                           t.tag_name,
                                           Checked =((from nt in db.NewTags where (nt.news_id == id) && (nt.tag_id == t.tag_id) select nt).Count() > 0)
                                       };
                    var newsproductcheck = from p in db.Products
                                           where p.status == "1"
                                           orderby p.product_name ascending
                                           select new
                                           {
                                               ...
                                               p.product_name,
                                               p.image,
                                               Checked = ((from np in db.NewProducts where (np.news_id == id) && (np.product_id == p.product_id) && (np.genre_id == p.GenreDetail.Genre.genre_id) select np).Count() > 0)
                                           };
                    var newsdto = new NewsDTO();
                    newsdto.news_id = id.Value;
                    newsdto.counttags = news.NewTags.Count();
                    //newsdto.countproducts = news.NewProduct.Count();
        ...
        public ActionResult NewsEdit(NewsDTO newsdto, string returnUrl)
        {
            ...
            try
            {
                ...
                news.update_by = User.Identity.GetEmail();
                news.status = newsdto.status;
                foreach (var item in db.NewTags)
                {
                ...
                foreach (var item in db.NewProducts)
                {
                    if (item.news_id == newsdto.news_id)
                    {
                        db.Entry(item).State = System.Data.Entity.EntityState.Deleted;
        ...
        public  JsonResult NewsDelete(int id)
        {
            ...
            if (User.Identity.Permiss_Delete() == true)
            {
                ...
                var newstag = db.NewTags.FirstOrDefault(a => a.news_id == id);
                var newsproduct = db.NewProducts.FirstOrDefault(m => m.news_id == id);
                ...
                {
                    if (news.NewTags.Count > 0)
                    {
                        db.NewTags.Remove(newstag);
                        db.SaveChanges();
                    }

                    //if (news.NewProducts.Count > 0)
                    //{
                    //    db.NewProducts.Remove(newsproduct);
                    //    db.SaveChanges();
                    //}
                    db.News.Remove(news);
                    db.SaveChanges();
