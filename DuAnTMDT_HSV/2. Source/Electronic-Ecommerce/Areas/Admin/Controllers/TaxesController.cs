...
namespace Electronic_Ecommerce.Areas.Admin.Controllers
{
    public class TaxesController : BaseController
    {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    if (!string.IsNullOrEmpty(search))
                    {
                        if (show.Equals("1"))//tìm kiếm tất cả
                           list = (IOrderedQueryable<Tax>)list.Where(s => s.taxes_id.ToString().Contains(search) || s.taxes_name.Contains(search));
                        else if (show.Equals("2"))//theo id
                            list = (IOrderedQueryable<Tax>)list.Where(s => s.taxes_id.ToString().Contains(search));
                        else if (show.Equals("3"))//theo tên thể loại
                            list = (IOrderedQueryable<Tax>)list.Where(s => s.taxes_name.Contains(search));
        ...
        //Thêm mới VAT
        [HttpPost]
        public JsonResult TaxesCreate(string name, int value, Tax taxes)
        {
            bool result;
            try
            {
