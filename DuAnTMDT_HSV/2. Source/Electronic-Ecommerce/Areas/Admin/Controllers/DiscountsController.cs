    ...
    public class DiscountsController : BaseController
    {
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    var list = from a in _db.Discounts
                               where (a.status == "1")
                               orderby a.discount_id descending
                               select a;
                    switch (sortOrder)
                    {
                        case "date_desc":
                        ...
                        case "discount_product":
                            ...
                            list = from a in _db.Discounts
                                   where ((a.status == "1") && a.discount_type == 1)
                                   orderby a.discount_id descending
                                   select a;
                            break;
                        case "discount_code":
                            ...
                            list = from a in _db.Discounts
                                   where ((a.status == "1") && a.discount_type == 2)
                                   orderby a.discount_id descending
                                   select a;
                            break;
                        case "discount_code_per":
                            ...
                            list = from a in _db.Discounts
                                   where ((a.status == "1") && a.discount_type == 3)
                                   orderby a.discount_id descending
                                   select a;
                            break;
                        default:
                    ...
                    switch (show)
                    {
                        //tìm kiếm tất cả
                        case "1":
                            list = (IOrderedQueryable<Discount>)list.Where(s => s.discount_id.ToString().Contains(search) || s.discount_price.ToString().Contains(search)  || s.discount_name.Contains(search) || s.discount_code.Contains(search));
                            break;
                        //theo id
                        case "2":
                            list = (IOrderedQueryable<Discount>)list.Where(s => s.discount_id.ToString().Contains(search));
                            break;
                        ...
                        case "5":
                            list = (IOrderedQueryable<Discount>)list.Where(s => s.discount_code.Contains(search));
                            break;
                    }
        ...
        public ActionResult DiscountTrash(string search, string show, int? size, int? page, string sortOrder)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    ViewBag.CodeSortParm = "discount_code";
                    ViewBag.CodePercentSortParm = "discount_code_per";
                    ViewBag.countTrash = _db.Discounts.Count(a => a.status == "0" && a.discount_type == 2);
                    ...
                    switch (sortOrder)
                    {
                        ...
                        case "discount_product":
                            ViewBag.sortname = "Xếp theo: Giảm trực tiếp sản phẩm";
                            list = from a in _db.Discounts
                                   where ((a.status == "0") && a.discount_type == 1)
                                   orderby a.discount_id descending
                                   select a;
                            break;
                        case "discount_code":
                            ViewBag.sortname = "Xếp theo: Code giảm giá theo giá tiền";
                            list = from a in _db.Discounts
                                   where ((a.status == "0") && a.discount_type == 2)
                                   orderby a.discount_id descending
                                   select a;
                            break;
                        case "discount_code_per":
                            ViewBag.sortname = "Xếp theo: Code giảm giá theo phần trăm";
                            list = from a in _db.Discounts
                                   where ((a.status == "0") && a.discount_type == 3)
                                   orderby a.discount_id descending
                                   select a;
                            break;
                        default:
                    ...
                    switch (show)
                    {
                        ...
                        case "1":
                            list = (IOrderedQueryable<Discount>)list.Where(s =>
                                s.discount_id.ToString().Contains(search) || s.discount_price.ToString().Contains(search)
        ...
        public ActionResult DiscountDetails(int? id)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                {
                    var discount = _db.Discounts.SingleOrDefault(a => a.discount_id == id);
        ...
        public ActionResult DiscountCreate(Discount discount)
        {
            try
            {
                ...
                var check_exist_dc = _db.Discounts.Any(m => m.discount_code == discount.discount_code && m.discount_code != null);
                if ((discount.discount_type == 2 || discount.discount_type == 3) && discount.discount_code == null)
                {
                    Notification.set_flash("Vui lòng nhập mã giảm giá", "danger");
                    return View(discount);
                }
                else if ((discount.discount_type == 2 || discount.discount_type == 3) && discount.quantity == null)
                {
                    Notification.set_flash("Vui lòng nhập số lượng", "danger");
                    return View(discount);
                }
                else if (discount.discount_type == 3 && discount.discount_price > 100)
                {
                    Notification.set_flash("Mức giảm phải nhỏ hơn hoặc bằng 100", "danger");
                    return View(discount);
                }
                else if ((discount.discount_type == 3) && discount.discount_max < 1000)
                {
                ...
                else
                {
                    ...
                    {
                        discount.discount_code = discount.discount_code.Trim();
                    }
        ...
        public JsonResult SendMailBirthDayCode(Discount discount)
        {
            ...
            try
            {
                ...
                foreach (Account accounts in account)
                {
                    ...
                    Discount checkcode = _db.Discounts.Where(m => m.discount_code == code).SingleOrDefault();
                    ...
                    if (checkcode == null && accounts.Dateofbirth.Month == thismonth.Month && accounts.create_at.Year <= lastyear.Year && accounts.role_id == 1 && accounts.Orders.Where(m=>m.status=="3").Count() > 0)
                    {
                       if (accounts.Orders.Sum(m => m.total)>= 50000000)
                        ...
                        string BirthDayCode = discount.discount_code;
        ...
        public JsonResult ChangeStatus(int id, int state = 0)
        {
            ...
            if (User.Identity.Permiss_Update() == true || User.Identity.Permiss_Modify() == true)
            {
                Discount discount = _db.Discounts.Where(m => m.discount_id == id).FirstOrDefault();
                int title = discount.discount_id;
