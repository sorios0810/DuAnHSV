...
namespace Electronic_Ecommerce.Areas.Admin.Controllers
{
    public class ParentGenresController : BaseController
    {
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    switch (show)
                    {
                        //tìm kiếm tất cả
                        case "1":
                            list = (IOrderedQueryable<ParentGenre>)list.Where(s =>
                                s.id.ToString().Contains(search) || s.name.Contains(search));
                            break;
                        //theo id
                        case "2":
                            list = (IOrderedQueryable<ParentGenre>)list.Where(s => s.id.ToString().Contains(search));
                            break;
                        //theo tên
                        case "3":
                            list = (IOrderedQueryable<ParentGenre>)list.Where(
        ...
        public ActionResult ParentGCreate(ParentGenre genre)
        {
        ...
        [ValidateInput(false)]
        [ValidateAntiForgeryToken]
        public ActionResult ParentGEdit(ParentGenre genre, string returnUrl)
        {
            try
            {
                genre.update_at = DateTime.Now;
        ...
        public JsonResult ChangeStatus(int id, int state = 0)
        {
            ...
            if (User.Identity.Permiss_Update() == true || User.Identity.Permiss_Modify() == true)
            {
                ParentGenre parent = _db.ParentGenres.Where(m => m.id == id).FirstOrDefault();
