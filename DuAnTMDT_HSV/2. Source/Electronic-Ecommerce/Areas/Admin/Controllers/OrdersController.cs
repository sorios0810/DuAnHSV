    ...
    public class OrdersController : BaseController
    {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    var list = from a in db.OrderDetails
                               ...
                               select new OrderDTO
                               {
                                   ...
                                   update_at = (DateTime)g.Key.b.update_at,
                    ...
                    {
                        case "name_asc":
                            list = from a in db.OrderDetails
                                   join b in db.Orders on a.order_id equals b.order_id
                                   group a by new { a.order_id, b } into g
                                   where g.Key.b.status != "0"
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    var list = from a in db.OrderDetails
                               ...
                               group a by new { a.order_id, b } into g
                               where g.Key.b.status == "0"
                               orderby (DateTime)g.Key.b.update_at descending
                               select new OrderDTO
                               {
                                   order_id = g.Key.order_id,
                                   total_price = g.Key.b.total,
        ...
        public ActionResult OrderDetails(int? id)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    var order_detail = db.OrderDetails.FirstOrDefault(m => m.order_id == id);
                    var check_discount = db.Discounts.FirstOrDefault(m => m.discount_code == order_detail.discount_code);
                    ...
                    var order = (from a in db.OrderDetails
                                 ...
                                 select new OrderDTO
                                 {
                                     ...
                                     update_at = (DateTime)(DateTime)g.Key.b.update_at,
                                     update_by = g.Key.b.update_by,
                                     Name = g.Key.b.OrderAddress.order_address_username,
                                     Phone = g.Key.b.OrderAddress.order_address_phonenumber,
                                     Address = g.Key.b.OrderAddress.order_address_content,
                                     discount_type = check_discount.discount_type,
                    ...
                    ViewBag.orderDetails = db.OrderDetails.Where(m => m.order_id == id).ToList();
        ...
        public void ExportDataToExcel(DateTime picker1 , DateTime picker2, string order__status,int payment__method)
        {
            ...
            using (ExcelEngine excelEngine = new ExcelEngine())
            {
                ...
                workbook.SaveAs("HSVTech_Report_"+ picker1.ToString("dd-MM-yyyy") + "_" + picker2.ToString("dd-MM-yyyy") + "_" + file + ".xlsx", ExcelSaveType.SaveAsXLS, HttpContext.ApplicationInstance.Response, ExcelDownloadType.Open);
            }
        }
