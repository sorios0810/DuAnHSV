...
using PagedList;
using static Electronic_Ecommerce.DTOs.RoleDTO;
using Domain;
    ...
    public class RolesController : BaseController
    {
        ...
        public ActionResult RolesTrash(string search, string show, int? size, int? page)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    var user = from a in db.Accounts
                               where a.status == "2"
                               orderby (DateTime)a.update_at descending // giảm dần
                               select a;
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                var list = from a in db.Orders
                           ...
                           select new OrderHistoryDTO
                           {
                               ...
                               update_at = (DateTime)a.update_at,
                    ...
                    case "date_asc":
                        list = from a in db.Orders
                               ...
                               select new OrderHistoryDTO
                               {
                                   ...
                                   total_price = a.total,
                                   create_at = a.create_at,
                                   update_at = (DateTime)(DateTime)a.update_at,
                                   payment_id = a.payment_id,
                                   payment_transaction = a.payment_transaction
                               };
                        break;
        ...
        public JsonResult RolesDelete(int id)
        {
            ...
            if (User.Identity.GetRole() == 4)
            {
                ...
                var newsProducts = db.NewProducts.FirstOrDefault(a => a.product_id == id);
                var product_image = db.ProductImages.FirstOrDefault(m => m.product_id == id);
                if (account.Orders.Count > 0)
                {
                    result = "ExitOrder";
                }
                else if (account.NewComments.Count > 0)
                {
                    result = "ExistComment";
                }
        ...
        public ActionResult RolesList(int ?page,int ? size)
        {
            if (User.Identity.IsAuthenticated)
            {
                if (User.Identity.GetRole() == 4)
                {
                    ...
                    var pageNumber = page ?? 1;
                    ViewBag.rolePermission = db.RolePermissions.ToList();
                    ...
                    var rolepermisscheck = from p in db.Permissions
                                           ...
                                           select new
                                           {
                                               p.permission_id,
                                               p.permission_name,
                                               Checked = ((from rp in db.RolePermissions where (rp.permission_id == p.permission_id) select rp).Count() > 0)
                                           };
        ...
        public JsonResult CreateRole(RoleDTO rolesDTO,Role roles)
        {
            bool result;
            if(User.Identity.GetRole() == 4)
            {
                ...
                foreach (var item in rolesDTO.RolePermissions)
                {
                    if (item.Checked)
                    {
                        db.RolePermissions.Add(new RolePermission() { role_id = roles.role_id, permission_id = item.permiss_id });
                        db.SaveChanges();
                    }
                }
        ...
        public JsonResult RoleEdit(RoleDTO rolesDTO)
        {
            bool result;
            if (User.Identity.GetRole() == 4)
            {
                Role roles = db.Roles.FirstOrDefault(m => m.role_id == rolesDTO.role_id);
                roles.update_at = DateTime.Now;
                roles.role_name = rolesDTO.role_name;
                foreach (var item in db.RolePermissions)
                {
