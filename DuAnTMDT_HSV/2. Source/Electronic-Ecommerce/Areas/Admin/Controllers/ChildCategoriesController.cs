    ...
    public class ChildCategoriesController : BaseController
    {
               ...
               User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    ViewBag.countTrash = db.ChildCategories.Count(a => a.status == "2");
                    var list = from a in db.ChildCategories
        ...
        public ActionResult ChildCateDetails(int? id)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ChildCategory newsCategory = db.ChildCategories.Find(id);
        ...
        public ActionResult ChildCateCreate()
        {
            ...
            {
                ViewBag.ListCategory = new SelectList(db.ParentCategories.Where(m => (m.status == "1")).OrderBy(m => m.name), "parentcategory_id", "name", 0);
                return View();
            }
            else
            {
        ...
        public ActionResult ChildCateCreate(ChildCategory ChillCategory)
        {
            ...
            try
            {
                var checkslug = db.ChildCategories.Any(m => m.slug == slug);
                if (checkslug)
                {
                ...
                db.ChildCategories.Add(ChillCategory);
                db.SaveChanges();
        ...
        public ActionResult ChildCateEdit(int? id,string returnUrl)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                var newscategory = db.ChildCategories.SingleOrDefault(a => a.childcategory_id == id);
        ...
        public ActionResult ChildCateDelete(int? id,string returnUrl)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                {
                    var chillCategory = db.ChildCategories.SingleOrDefault(a => a.childcategory_id == id);
        ...
        {
            var chillCategory = db.ChildCategories.SingleOrDefault(a => a.childcategory_id == id);
            db.ChildCategories.Remove(chillCategory);
            db.SaveChanges();
