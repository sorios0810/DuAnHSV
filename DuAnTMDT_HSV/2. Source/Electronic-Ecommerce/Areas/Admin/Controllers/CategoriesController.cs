    ...
    public class CategoriesController : BaseController
    {
        ...
        public ActionResult CategoriesIndex(string search, string show, int? size, int? page)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    ViewBag.countTrash = db.ParentCategories.Count(a => a.status == "2");
                    var list = from a in db.ParentCategories
        ...
        public ActionResult CategoriesDetails(int ? id)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    var category = db.ParentCategories.Where(m => m.parentcategory_id == id).FirstOrDefault();
        ...
        public ActionResult CategoriesCreate(ParentCategory category)
        {
            ...
            try
            {
                var checkslug = db.ParentCategories.Any(m => m.slug == category.slug);
                if (checkslug)
                {
                ...
                db.ParentCategories.Add(category);
                db.SaveChanges();
        ...
        public ActionResult CategoriesEdit(int? id,string returnUrl)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                var category = db.ParentCategories.SingleOrDefault(a => a.parentcategory_id == id);
        ...
        {
            var category = db.ParentCategories.SingleOrDefault(a => a.parentcategory_id == id);
            db.ParentCategories.Remove(category);
            db.SaveChanges();
