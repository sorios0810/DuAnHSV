...
namespace Electronic_Ecommerce.Areas.Admin.Controllers
{
    public class DashboardController : BaseController
    {
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                switch (sortOrder)
                {
                    case "today"://sort ngày hiện tại
                        ...
                        ViewBag.CountquantitySales = db.OrderDetails.Where(m => m.Order.status != "0" && m.Order.oder_date.Day == today.Day && m.Order.oder_date.Month == today.Month).Sum(m => (int?)m.quantity) ?? 0;
                    ...
                    case "7daysago":
                        ...
                        ViewBag.CountquantitySales = db.OrderDetails.Where(m => m.Order.status != "0" && m.Order.oder_date > start_date && m.Order.oder_date < end_date).Sum(m => (int?)m.quantity) ?? 0;
                    ...
                    case "month":
                        ...
                        ViewBag.CountquantitySales = db.OrderDetails.Where(m => m.Order.status != "0" && m.Order.oder_date.Month == today.Month && m.Order.oder_date.Year == today.Year).Sum(m => (int?)m.quantity) ?? 0;
                    ...
                    case "year":
                        ...
                        ViewBag.CountquantitySales = db.OrderDetails.Where(m => m.Order.status != "0" && m.Order.oder_date.Year == today.Year).Sum(m => (int?)m.quantity) ?? 0;
                    ...
                    default:
                        ...
                        ViewBag.CountTotalOrder = db.Orders.Where(m => m.status != "0" && m.oder_date.Day == today.Day && m.oder_date.Month == today.Month && m.oder_date.Year == today.Year).Count();
                        ViewBag.CountquantitySales = db.OrderDetails.Where(m => m.Order.status != "0" && m.Order.oder_date.Day == today.Day && m.Order.oder_date.Month == today.Month && m.Order.oder_date.Year == today.Year).Sum(m => (int?)m.quantity) ?? 0;
        ...
        public ActionResult CountQuantityProductlOrder(int? id)
        {
            ...
            {
                case 1:
                    query = db.OrderDetails.Include("Product")
                   .Where(m => m.Order.status != "0" && m.Order.oder_date.Day == today.Day && m.Order.oder_date.Month == today.Month && m.Order.oder_date.Year == today.Year)
                   .GroupBy(p => p.Product.product_name)
                   .Select(g => new { name = g.Key, count = g.Sum(w => w.quantity) }).OrderByDescending(m => m.count).Take(5).ToList();
                    break;
