    ...
    public class Product_ImageController : BaseController
    {
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    var list = from a in db.Products
                               join c in db.Genres on a.GenreDetail.Genre.genre_id equals c.genre_id
                               join i in db.ProductImages on a.product_id equals i.product_id
        ...
        public JsonResult GetProductImageSearch(string Prefix)
        {
            var search = (from c in db.Products
                join b in db.ProductImages on c.product_id equals b.product_id
        ...
        public ActionResult ProductImgEdit(int? id,string returnUrl)
        {
            if (User.Identity.IsAuthenticated)
            {
                if (User.Identity.Permiss_Modify() == true)
                {
                    ...
                    var productimg = db.ProductImages.Where(m => m.product_id == id).FirstOrDefault();
                    if (productimg == null)
                    {
                        return HttpNotFound();
                    }
        ...
        public ActionResult ProductImgEdit(ProductImage product_Image, string returnUrl,int id)
        {
            var productimg = db.ProductImages.FirstOrDefault(m => m.product_id == id);
