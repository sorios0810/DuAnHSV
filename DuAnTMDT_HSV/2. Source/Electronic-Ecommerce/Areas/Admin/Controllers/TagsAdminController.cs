...
namespace Electronic_Ecommerce.Areas.Admin.Controllers
{
    public class TagsAdminController : BaseController
    {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    if (!string.IsNullOrEmpty(search))
                    {
                        if (show.Equals("1"))//tìm kiếm tất cả
                            list = (IOrderedQueryable<Tag>)list.Where(s => s.tag_id.ToString().Contains(search) || s.tag_name.Contains(search));
                        else if (show.Equals("2"))//theo id
                            list = (IOrderedQueryable<Tag>)list.Where(s => s.tag_id.ToString().Contains(search));
                        else if (show.Equals("3"))//theo tên thể loại
                            list = (IOrderedQueryable<Tag>)list.Where(s => s.tag_name.Contains(search));
        ...
        //Thêm tag mới
        [HttpPost]
        public JsonResult TagCreate(string name, Tag tags)
        {
            bool result;
