...
namespace Electronic_Ecommerce.Controllers
{
    public class ProductController : Controller
    {
        ...
        // GET: Product
        //Navigation Menu: Lớp cha bao gồm: lớp con > danh mục con
        public ActionResult NavigationBar()
        {
            // Lấy danh sách thương hiệu từ stored procedure
            var listBrand = db.Database.SqlQuery<Brand>("exec sp_GetBrandList").ToList();
            ViewBag.Brands = listBrand;

            // Lấy danh sách các mục cha từ cơ sở dữ liệu
            List<ParentGenre> parents = db.ParentGenres.Where(m => m.status == "1").Take(10).ToList();

            // Lấy danh sách các thể loại có danh mục con từ cơ sở dữ liệu
            var genresWithChildren = db.Genres.Where(g => g.GenreDetails.Any(d => d.status == "1")).ToList();

            // Thêm danh sách các danh mục con cho từng thể loại
            foreach (var genre in genresWithChildren)
            {
                genre.GenreDetails = db.GenreDetails.Where(d => d.genre_id == genre.parent_genre_id && d.status == "1").ToList();
            }

            // Thêm danh sách thể loại có danh mục con cho từng mục cha
            foreach (var parent in parents)
            {
                parent.Genres = genresWithChildren.Where(g => g.parent_genre_id == parent.id).ToList();
            }


            return PartialView("NavigationBar", parents);
        }
        ...
        public ActionResult ListProduct(int? page, string slug, string sortOrder)
        {
            ...
            var product = db.Products.Where(m => m.GenreDetail.status == "1" && m.GenreDetail.Genre.status == "1" && m.GenreDetail.Genre.ParentGenre.status == "1" && m.status == "1" && m.Brand.status == "1" && (m.GenreDetail.Genre.ParentGenre.slug == slug || m.GenreDetail.Genre.slug == slug || m.GenreDetail.slug == slug || m.Brand.slug == slug)).FirstOrDefault();
            if (product.GenreDetail.Genre.slug == slug)
            {
                ViewBag.Type = product.GenreDetail.Genre.genre_name;
            ...
            else if (product.GenreDetail.slug == slug)
            {
                ViewBag.Type = product.GenreDetail.genre_detail_name;
                ViewBag.Title = ViewBag.Type;
            }
            else if (product.GenreDetail.Genre.ParentGenre.slug == slug)
            {
                ViewBag.Type = product.GenreDetail.Genre.ParentGenre.name;
                ViewBag.Title = ViewBag.Type;
            }
            ViewBag.AvgFeedback = db.Feedbacks.ToList();
            ViewBag.OrderFeedback = db.OrderDetails.ToList();
            ViewBag.Countproduct = db.Products.Where(m => m.status == "1" && m.GenreDetail.Genre.ParentGenre.status == "1" && m.GenreDetail.Genre.status == "1" && m.GenreDetail.status == "1" && m.Brand.status == "1" && (m.GenreDetail.Genre.ParentGenre.slug == slug || m.GenreDetail.Genre.slug == slug || m.GenreDetail.slug == slug || m.Brand.slug == slug)).Count();
            ...
            BannerGlobal();
            return View("Product", GetProduct(m => (m.GenreDetail.Genre.ParentGenre.slug == slug || m.GenreDetail.Genre.slug == slug || m.GenreDetail.slug == slug || m.Brand.slug == slug) && m.GenreDetail.Genre.ParentGenre.status == "1" && m.GenreDetail.Genre.status == "1" && m.GenreDetail.status == "1" && m.status == "1" && m.Brand.status == "1", page, sortOrder));
        }
        ...
        public ActionResult SearchResult(int? page, string sortOrder, string s)
        {
            ...
            switch (sortOrder)
            {
                case "duoi-3-trieu":
                    ...
                    list = (IOrderedQueryable<Product>)db.Products.OrderByDescending(m => m.product_id).Where(m => ((m.selling_price - m.Discount.discount_price) < 3000000) && m.status == "1" && m.product_name.Contains(s));
                    break;
                case "tu-3-8-trieu":
                    ViewBag.sortname = "Từ 3 đến 8 triệu";
                    list = (IOrderedQueryable<Product>)db.Products.OrderByDescending(m => m.product_id).Where(m => ((m.selling_price - m.Discount.discount_price) >= 3000000) && ((m.selling_price - m.Discount.discount_price) <= 8000000) && m.status == "1" && m.product_name.Contains(s));
                    break;
                case "tu-8-15-trieu":
                    ViewBag.sortname = "Từ 8 đến 15 triệu";
                    list = (IOrderedQueryable<Product>)db.Products.OrderByDescending(m => m.product_id).Where(m => (m.selling_price - m.Discount.discount_price) > 8000000 && (m.selling_price - m.Discount.discount_price) <= 15000000);
                    break;
                case "tu-15-25-trieu":
                    ViewBag.sortname = "Từ 15 đến 25 triệu";
                    list = (IOrderedQueryable<Product>)db.Products.OrderByDescending(m => m.product_id).Where(m => (m.selling_price - m.Discount.discount_price) > 15000000 && (m.selling_price - m.Discount.discount_price) <= 25000000);
                    break;
                case "tren-25-trieu":
                    ViewBag.sortname = "Trên 25 triệu";
                    list = (IOrderedQueryable<Product>)db.Products.OrderByDescending(m => m.product_id).Where(m => (m.selling_price - m.Discount.discount_price) > 25000000);
                    break;
                case "view_desc":
                ...
                case "price_asc":
                    ...
                    list = (IOrderedQueryable<Product>)db.Products.OrderBy(m => (m.selling_price - m.Discount.discount_price)).Where(m => m.status == "1" && m.product_name.Contains(s));
                    break;
                case "price_desc":
                    ViewBag.sortname = "Giá giảm dần";
                    list = (IOrderedQueryable<Product>)db.Products.OrderByDescending(m => m.selling_price - m.Discount.discount_price).Where(m => m.status == "1" && m.product_name.Contains(s));
                    break;
                case "discount_asc":
        ...
        {
            //var product = db.Database.SqlQuery<ProductDTO>("exec sp_taolao").Where(p => p.slug == slug).FirstOrDefault();
            var product = (from p in db.Products
                           join g in db.Genres on p.GenreDetail.Genre.genre_id equals g.genre_id
                           join gd in db.GenreDetails on p.GenreDetail.Genre.genre_id equals gd.genre_id
                           join pg in db.ParentGenres on g.parent_genre_id equals pg.id
                           join br in db.Brands on p.brand_id equals br.brand_id
                           join inv in db.Inventories on p.product_id equals inv.product_id
                           join dc in db.Discounts on p.discount_id equals dc.discount_id
                           join pig in db.ProductImages on p.product_id equals pig.product_id
                           where p.slug == slug && p.status == "1" && p.GenreDetail.Genre.status == "1" && p.GenreDetail.Genre.ParentGenre.status == "1" && br.status == "1"
                           ...
                           select new ProductDTO
                           {
                               ...
                               genre_slug = g.slug,
                               genre_detail_id = gd.genre_detail_id,
                               genre_detail_name = gd.genre_detail_name,
            ...
            //xử lý phần feedback
            ViewBag.CheckEditOrder = db.OrderDetails.Where(m => m.Product.slug == slug && m.Order.account_id == UserId && m.Order.status == "3").Count();
            ...
            List<FeedbackImage> feedbackimages = db.FeedbackImages.ToList();
            ...
            ViewBag.CheckExistBannerDetail = db.BannerDetails.Where(m => m.product_id == product.product_id).FirstOrDefault();
            ViewBag.BannerDetail = db.BannerDetails.Where(m => m.product_id == product.product_id).ToList();
            ViewBag.Banner = db.Banners;
            //xử lý discount global
            List<Discount> CouponGlobal = db.Discounts.Where(m => m.discount_start < DateTime.Now && m.discount_end > DateTime.Now && m.quantity != "0" && (m.discount_type == 2 || m.discount_type == 3) && m.status == "1" && m.discount_global == "1").ToList();
            ViewBag.CouponGlobal = CouponGlobal;
            //tin tức của sản phẩm
            List<NewProduct> newsproducts = db.NewProducts.Where(m => m.product_id == product.product_id).ToList();
            ViewBag.newsproducts = newsproducts;
            //Sản phẩm liên quan
            List<Product> relatedproduct = db.Products.Where(item => item.status == "1" && item.product_id != product.product_id && item.GenreDetail.genre_id == product.genre_id).Take(4).ToList();
        ...
        {           
            ...
            var search = (from c in db.Products
                          where c.status == "1" && (c.product_name.Contains(Prefix) || c.product_id.ToString().Contains(Prefix) || c.GenreDetail.Genre.ParentGenre.name.Contains(Prefix) || c.GenreDetail.Genre.genre_name.Contains(Prefix) || c.Brand.brand_name.Contains(Prefix) || c.specification.Contains(Prefix))
                          ...
                          select new ProductDTO
                          {
                              ...
                              slug = c.slug,
                              Image = c.image,
                              price = (c.selling_price - c.Discount.discount_price),
                          });
        ...
        private IPagedList GetProduct(Expression<Func<Product, bool>> expr, int? page, string sortOrder)
        {
            ...
            switch (sortOrder)
            {
                case "duoi-3-trieu":
                    ViewBag.sortname = "Dưới 3 triệu";
                    list = db.Products.Where(expr).OrderByDescending(m => m.product_id).Where(m => (m.selling_price - m.Discount.discount_price) < 3000000).ToPagedList(pageNumber, pageSize);
                    ViewBag.Countproduct = list.Count();
                    break;
                case "tu-3-8-trieu":
                    ViewBag.sortname = "Từ 3 đến 8 triệu";
                    list = db.Products.Where(expr).OrderByDescending(m => m.product_id).Where(m => (m.selling_price - m.Discount.discount_price) >= 3000000 && (m.selling_price - m.Discount.discount_price) <= 8000000).ToPagedList(pageNumber, pageSize);
                    ViewBag.Countproduct = list.Count();
                    break;
                case "tu-8-15-trieu":
                    ViewBag.sortname = "Từ 8 đến 15 triệu";
                    list = db.Products.Where(expr).OrderByDescending(m => m.product_id).Where(m => (m.selling_price - m.Discount.discount_price) > 8000000 && (m.selling_price - m.Discount.discount_price) <= 15000000).ToPagedList(pageNumber, pageSize);
                    ViewBag.Countproduct = list.Count();
                    break;
                case "tu-15-25-trieu":
                    ViewBag.sortname = "Từ 15 đến 25 triệu";
                    list = db.Products.Where(expr).OrderByDescending(m => m.product_id).Where(m => (m.selling_price - m.Discount.discount_price) > 15000000 && (m.selling_price - m.Discount.discount_price) <= 25000000).ToPagedList(pageNumber, pageSize);
                    ViewBag.Countproduct = list.Count();
                    break;
                case "tren-25-trieu":
                    ViewBag.sortname = "Trên 25 triệu";
                    list = db.Products.Where(expr).OrderByDescending(m => m.product_id).Where(m => (m.selling_price - m.Discount.discount_price) > 25000000).ToPagedList(pageNumber, pageSize);
                    ViewBag.Countproduct = list.Count();
                    break;
                case "view_desc":
                    ViewBag.sortname = "Xem nhiều";
                ...
                case "price_asc":
                    ViewBag.sortname = "Giá tăng dần";
                    list = db.Products.Where(expr).OrderBy(m => (m.selling_price - m.Discount.discount_price)).ToPagedList(pageNumber, pageSize);
                    break;
                case "price_desc":
                    ViewBag.sortname = "Giá giảm dần";
                    list = db.Products.Where(expr).OrderByDescending(m => (m.selling_price - m.Discount.discount_price)).ToPagedList(pageNumber, pageSize);
                    break;
                case "discount_asc":
                    ViewBag.sortname = "Khuyến mãi tăng dần";
        ...
        //Đánh giá sản phẩm
        [HttpPost]
        public ActionResult RatingProduct(Feedback feedback, FeedbackImage feedback_Image)
        {
            try
            {
                var jsonUserData = User.Identity.Name;
                ...
                //xử lý phần feedback
                ViewBag.CheckEditOrder = db.OrderDetails.Where(m => m.product_id == feedback.product_id && m.Order.account_id == UserId && m.Order.status == "3").Count();
                ...
                foreach (HttpPostedFileBase image_multi in feedback_Image.UploadImageMultiple)
                {
                    if (image_multi != null)
                    {
                        ...
                        fileName = fileName + DateTime.Now.ToString("dd-MM-yyy:ss") + new Random().Next(1000) + extension;
                        ...
                        db.FeedbackImages.Add(feedback_Image);
                        db.SaveChanges();
                    }
                    else
                    {
