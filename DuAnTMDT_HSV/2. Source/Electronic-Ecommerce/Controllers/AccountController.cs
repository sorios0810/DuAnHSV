...
using ChangePassword = Domain.Entities.ChangePassword;
...
namespace Electronic_Ecommerce.Controllers
{
    public class AccountController : Controller
    {
        ...
        private readonly CmsDbContext db = new CmsDbContext();
        private readonly string ApiKey = "AIzaSyBmz7vEyB1nOctzPQVSDW2OLO-LHC95OW8";
        private static string Bucket = "hsvcompany-2023.appspot.com";
        private static string AuthEmail = "hsvcompany.tech@gmail.com";
        private static string AuthPassword = "hsvcompany";
        ...
        public ActionResult SignIn(SignIn model, string returnUrl)
        {
            ...
            {
                List<RolePermission> permissions = DataItem.Role.RolePermissions.ToList();
                ...
                var userData = new LoggedUserData
                {
                    ...
                    RoleCode = DataItem.Role.role_id,
                    Rolename = DataItem.Role.role_name,
        ...
        //Gửi email xác thựctrang này chỉ tồn tại nếu bạn vừa dk xong, nếu bạn refresh lại trang thì trang này sẽ không hiện nữa
        ...
        public void SendVerificationLinkEmail(string emailID, string activationCode, string emailFor)
        {
            ...
            string body = System.IO.File.ReadAllText(HostingEnvironment.MapPath("~/EmailTemplate/") + "EmailConfirm" + ".cshtml");
        ...
        public ActionResult AddressManager()
        {
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                var listaddress = (from a in db.AccountAddresses
                                   ...
                                   select new AddressManagerDTO
                                   {
                                       ...
                                       province_name = a.Province.province_name,
                                       province_type = a.Province.type,
                                       district_name = a.District.district_name,
                                       district_type = a.District.type,
                                       ward_name = a.Ward.ward_name,
                                       ward_type = a.Ward.type,
        ...
        public ActionResult EditProfile()
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                ViewBag.AccountName = account.Name;
                BannerGlobal();
                return View(account); // truyền account vào View
            }
            else
            {
        ...
        public ActionResult TrackingOrder(int? page, int? size, string search, string sortOrder)//kiểm tra đơn hàng đã đặt và có phân trang cho phần đơn hàng dã đặt
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                ViewBag.ProcessingSortParm = sortOrder == "order_processing" ? "order_processing" : "order_processing";
                ViewBag.CompleteSortParm = sortOrder == "order_complete" ? "order_complete" : "order_complete";
                ViewBag.CancelSortParm = sortOrder == "order_cancel" ? "order_cancel" : "order_cancel";
                ...
                ViewBag.itemOrder = db.OrderDetails.OrderByDescending(m => m.order_id);
                ...
                switch (sortOrder)
                {
                    ...
                    case "date_desc":
                        ...
                        break;
                    case "price_asc":
                        ViewBag.sortname = "Trị giá thấp - cao";
                        list = from a in db.Orders
                               where (a.account_id == userid)
                        ...
                        break;
                    case "price_desc":
                        ViewBag.sortname = "Trị giá cao - thấp";
                        list = from a in db.Orders
                               where (a.account_id == userid)
        ...
        public ActionResult TrackingOrderDetail(int id)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                ViewBag.CheckEditOrder = db.OrderDetails.Where(m => m.Order.account_id == user_id && m.Order.status == "3").ToList();
                int userid = User.Identity.GetUserId();
                var orderdetail = db.Orders.FirstOrDefault(m => m.order_id == id && m.account_id == userid);
                if (orderdetail != null)
                {
                    var discountpriceorder = db.OrderDetails.FirstOrDefault(m => m.order_id == id);
                    var discount = db.Discounts.Where(m => m.discount_code == discountpriceorder.discount_code).FirstOrDefault();
                    if (discount == null)
                    {
                    ...
                    OrderDTO order = new OrderDTO
                    {
                        ...
                        temporary = orderdetail.OrderDetails.Sum(m => m.price * m.quantity),
                        discount_price = discount.discount_price,
                        discount_type = discount.discount_type,
                        discount_max = discount.discount_max
                    };
                    var account = db.Accounts.Where(m => m.account_id == userid).FirstOrDefault();
                    List<OrderDetail> product_orderdetail = db.OrderDetails.Where(m => m.order_id == id).ToList();
        ...
        [HttpPost]
        public ActionResult ChangeOrderAddress(OrderDTO model, int id, string address_content, string phonenumber, string username, string ward, string district, string province)
        {
            bool result;
            try
            {
                var order_adress = db.OrderAddresses.FirstOrDefault(m => m.order_address_id == id);
        ...
        //Huỷ đơn hàng
        [HttpPost]
        public ActionResult CancelOrder(OrderDTO model)
        {
            bool result = false;
        ...
        public ActionResult CommentHistory(int? page, int? size)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                ViewBag.AccountName = account.Name;
                var pageNumber = page ?? 1;
                var list = (from nc in db.NewComments
        ...
        public ActionResult ReplyHistory(int? page)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                ViewBag.AccountName = account.Name;
                var pageNumber = page ?? 1;
                var list = (from rc in db.ReplyComments
                            join nc in db.NewComments on rc.comment_id equals nc.comment_id
                            join ns in db.News on nc.news_id equals ns.news_id
        ...
        public ActionResult FeedbackHistory(int? page, string sortOrder)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                ViewBag.cancelSortParm = sortOrder == "cancel" ? "cancel" : "cancel";
                int userid = User.Identity.GetUserId();
                var account = db.Accounts.Where(m => m.account_id == userid).FirstOrDefault();
                ViewBag.fb_img = db.FeedbackImages.ToList();
                ViewBag.Avatar = account.Avatar;
                ViewBag.AccountName = account.Name;
                var pageNumber = page ?? 1;
                var list = from fb in db.Feedbacks
                           ...
                           select new FeedbackDTO
                           {
                               ...
                               genre_id = p.GenreDetail.Genre.genre_id,
