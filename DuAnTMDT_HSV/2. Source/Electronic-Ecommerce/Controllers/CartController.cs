...
using Domain;
using Electronic_Ecommerce.ZaloPay;
using Electronic_Ecommerce.ZaloPay.Models;
...
namespace Electronic_Ecommerce.Controllers
{
    public class CartController : Controller
    {
        ...
        //    {
        //        var products = cart.Select(c => new
        //        {
        ...
        {
            List<CartItem> lst = Session["Cart"] as List<CartItem>;
            CartItem item = lst.Find(n => n.product_id == id);
            lst.Remove(item);
        ...
        [Authorize]
        public ActionResult Checkout(CheckoutOrderDTO checkout)
        {
            ...
            var listaddress = db.AccountAddresses.Where(m => m.account_id == userId).OrderByDescending(m => m.account_address_default).ToList();
            ...
            var user = db.AccountAddresses.SingleOrDefault(u => u.account_id == userId && u.account_address_default == true);
            if (user != null)
            {
                checkout.checkout_username = user.account_address_username;
                checkout.checkout_phone_number = user.account_address_phonenumber;
                checkout.checkout_email = user.Account.Email;
                checkout.checkout_address_province = user.Province.province_name;
                checkout.checkout_address_district = user.District.type + ' ' + user.District.district_name;
                checkout.checkout_address_ward = user.Ward.type + ' ' + user.Ward.ward_name;
                checkout.checkout_address_content = user.account_address_content;
                checkout.checkout_address = user.account_address_content + ", " + user.Ward.type + " " + user.Ward.ward_name + ", " + user.District.type + " " + user.District.district_name + ", " + user.Province.province_name;
            }
            ...
            {
                var item = products[i];
                productPrice = item.Product.selling_price;
                //kiểm tra discount code
                if (item.Product.Discount != null)
                {
                    if (item.Product.Discount.discount_start < DateTime.Now && item.Product.Discount.discount_end > DateTime.Now)
                    {
                        productPrice = item.Product.selling_price - item.Product.Discount.discount_price;
                    }
                }
            ...
            if (Session["Discount"] != null && Session["Discountcode"] != null)
            {
                ...
                var oderdetail = db.OrderDetails.Any(m => m.Order.account_id == userid && code == m.discount_code);
                if (oderdetail)
                {
        ...
        public ActionResult ChangeAdressOrder(AccountAddress model)
        {
            bool result;
            if (User.Identity.IsAuthenticated)
            {
                var address = db.AccountAddresses.FirstOrDefault(m => m.account_address_id == model.account_address_id);
        ...
        public async Task<ActionResult> SaveOrder(CheckoutOrderDTO checkout, OrderAddress orderAddress, string check_discount, string SubjectMail, string ButtonConfirmlink, string ButtonConfirm, string OrderPayment, string OrderDelivery, string DeliveryId, string SubOrderTotal, string Discount_Price, string ProductOrder, string OrderId, string OrderStatus, string OrderTotal, string UserEmail, string UserName, string UserPhoneNumber, string UserAddress)
        {
            ...
            try
            {
                if (Session["Discount"] != null && Session["Discountcode"] != null)
                {
                    ...
                    var discountupdatequan = db.Discounts.Where(d => d.discount_code == check_discount).FirstOrDefault();
                ...
                if (checkout.checkout_username != null && checkout.checkout_username != null && checkout.checkout_address_content != null
                    && checkout.checkout_address_district != null && checkout.checkout_address_ward != null && checkout.checkout_address_province != null && checkout.checkout_phone_number != null)
                {
                    orderAddress.order_address_username = checkout.checkout_username;
                    orderAddress.order_adress_province = checkout.checkout_address_province;
                    orderAddress.order_adress_district = checkout.checkout_address_district;
                    orderAddress.order_adress_wards = checkout.checkout_address_ward;
                    orderAddress.order_address_content = checkout.checkout_address_content;
                    orderAddress.order_address_phonenumber = checkout.checkout_phone_number;
                ...
                for (int i = 0; i < cart.Item1.Count; i++)
                {
                    ...
                    var _price = item.Product.selling_price;
                    if (item.Product.Discount != null)
                    {
                        //ngày bắt đầu phải nhỏ hơn ngày hiên tại, và ngày kết thúc phải lớn ngày hiện tại
                        if (item.Product.Discount.discount_start < DateTime.Now && item.Product.Discount.discount_end > DateTime.Now)
                        {
                            _price = item.Product.selling_price - item.Product.Discount.discount_price;
                        }
                    }
                    ...
                    order.OrderDetails.Add(new OrderDetail
                    {
                        create_at = DateTime.Now,
                        create_by = User.Identity.GetUserId().ToString(),
                        genre_id = item.Product.GenreDetail.Genre.genre_id,
                        price = _price,
                    ...
                    var inventory = db.Inventories.SingleOrDefault(inv => inv.product_id == item.product_id);
                    productquancheck = inventory.quantity_on_hand.ToString(); // sử dụng toán tử null-conditional và giá trị mặc định 0
                    product.buyturn += cart.Item2[i];
                    inventory.quantity_on_hand = (inventory?.quantity_on_hand ?? 0) - cart.Item2[i]; // sử dụng toán tử null-conditional và giá trị mặc định 0

                    double price_product_email = product.selling_price;
                    if (product.Discount.discount_start < DateTime.Now && product.Discount.discount_end > DateTime.Now && product.Discount.status == "1")
                    {
                        price_product_email = product.selling_price - product.Discount.discount_price;
                    }
                ...
                switch (checkout.payment_id)
                {
                    case 2://thanh toán bằng vnpay
                        return RedirectToAction("vnpay", "PaymentMethod");
                    case 5://thanh toán bằng zalo pay
                        var amount = Convert.ToInt64(TempData["Total"]);
                        var description = "HSV - Thanh toán đơn hàng #" + order.order_id;
                    ...
                    case 6://thanh toán bằng paypal
                        return RedirectToAction("PaymentWithPaypal", "PaymentMethod");
                    default:
        ...
        public void SendVerificationLinkEmail(string SubjectMail, string ButtonConfirmlink, string ButtonConfirm, string OrderPayment, string OrderDelivery, string DeliveryId, string SubOrderTotal, string Discount_Price, string UserEmail, string UserName, string UserPhoneNumber, string UserAddress, string OrderId, string OrderStatus, string OrderTotal, string ProductOrder)
        {
            ...
            string subject;
            string body = System.IO.File.ReadAllText(HostingEnvironment.MapPath("~/EmailTemplate/") + "EmailOrder" + ".cshtml"); //dùng body mail html , file template nằm trong thư mục "EmailTemplate/Text.cshtml"
            subject = SubjectMail;
        ...
        public ActionResult UseDiscountCode(string code, double total_price)
        {
            ...
            if (discount != null)
            {
                ...
                if (discount.discount_start < DateTime.Now && discount.discount_end > DateTime.Now && discount.quantity != "0")
                {
                    ...
                    //sestion code
                    Session["Discountcode"] = discount.discount_code.Trim();
        ...
        private Tuple<List<Inventory>, List<int>> GetCart()
        {
            //check null 
            ...
            foreach (var item in cart)
            {
                ...
                if (int.TryParse(tempArr[1], out int productId))
                {
                ...
                int.TryParse(cValue, out int quantityValue);
                quantities.Add(quantityValue);

            }
            // Select sản phẩm để hiển thị
            var listProduct = new List<Inventory>();
            foreach (var id in productIds)
            {
                var product = db.Inventories.FirstOrDefault(p => p.Product.status == "1" && p.product_id == id);
                if (product != null)
                {
                    listProduct.Add(product);
                }
                else
                {
                    ...
                    // Kiểm tra nếu chỉ mục là hợp lệ trước khi thực hiện xóa phần tử
            ...
            return new Tuple<List<Inventory>, List<int>>(listProduct, quantities);//lấy id sản phẩm và số lượng
        }
