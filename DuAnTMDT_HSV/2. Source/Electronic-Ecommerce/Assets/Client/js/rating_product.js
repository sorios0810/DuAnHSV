...
function CRateOut(rating) {
    for (var i = 1; i <= rating; i++) {
        $("#rate" + i).attr('class', 'icon-star-o');
    }
}
//di chuột vào một đối tượng
function CRateOver(rating) {
    for (var i = 1; i <= rating; i++) {
        $("#rate" + i).attr('class', 'icon-star');
    }
}
//click vào đối tượng
...
//set số lượng upload ảnh, và kích thước file
...
function AjaxPost2(formData)//get data trong onsubmit="AjaxPost(this)"
{
    $.ajax({
        type: 'post',
        url: '/Product/RatingProduct',//gọi đến action xử lý lưu dữ liệu xuống database
