...
$(".js-delete").click(function (ev) {
	bootbox.confirm({
		message: "Bạn chắc chắn xoá sản phẩm ?",
		buttons: {
			confirm: {
				label: 'Xoá',
			...
			cancel: {
				label: 'Quay lại',
				className: 'btn-outline-secondary text-uppercase'
			}
		},
			...
			if (!isLogged) {
				...
				bootbox.confirm({
					...
					buttons: {
						confirm: {
							label: 'Đăng nhập',
							className: 'btn-outline-info text-uppercase'
						},
						cancel: {
							label: 'Quay lại',
							className: 'btn-outline-secondary text-uppercase'
...
$(".btn-decrement").click(function (ev) {
	$(".inc").removeClass('no-pointer-events');
...
$(".btn-increment").click(function (ev) {
	...
	else if (quan >= maxquan)
	{
		...
		$(".inc").addClass('no-pointer-events');
		const Toast = Swal.mixin({
			toast: true,
			position: 'top',
			showConfirmButton: false,
